...
public delegate void NetworkActionListenerDelegate(Action action, SFSErrorCode errorCode, ISFSObject packet);
...
public class NetworkController : MonoBehaviour
{
    ...
    public static void SendSurrenderPVEToSever()
    {
        ...
        Send(Action.PVE_SURRENDER, sfsObject);
    }
    ...
    protected static void OnExtentionResponse(BaseEvent evt)
    {
        ...
        if (cmd == CLIENT_REQUEST)
        {
            ...
            Action action = (Action)packet.GetInt(ACTION_INCORE);
    ...
    public static void Send(Action action, ISFSObject data)
    {
    ...
    protected static void OnReceiveServerAction(Action action, SFSErrorCode errorCode, ISFSObject packet)
    {

        switch (action)
        {

            case Action.COMBAT_BOT:
                {
                    if (errorCode == SFSErrorCode.SUCCESS)
                    {
            ...
            case Action.LOAD_LIST_HERO_INFO:
                {
                    if (errorCode == SFSErrorCode.SUCCESS)
                    {
            ...
            case Action.PVP_COMBAT_PREPARE:
                {
                    if (errorCode == SFSErrorCode.SUCCESS)
                    {
            ...
            case Action.GET_LINEUP_SLOT_PACK:
                {
                    SceneTransition.Instance.ShowWaiting(false);
                    if (errorCode == SFSErrorCode.SUCCESS)
                    {
            ...
            case Action.TRAIN_SAILORS_REMAIN:
                {
                    SceneTransition.Instance.ShowWaiting(false);
                    if (errorCode == SFSErrorCode.SUCCESS)
                    {
            ...
            case Action.USER_LIST_ARENA_REWARD:
                {
                    SceneTransition.Instance.ShowWaiting(false);
                    if (errorCode == SFSErrorCode.SUCCESS)
                    {
            ...
            case Action.GET_USER_CLAIMED_FRAGMENTS:
                {
