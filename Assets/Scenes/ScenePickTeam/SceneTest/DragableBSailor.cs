...
public class DragableBSailor : MonoBehaviour
{
    ...
    protected void OnMouseUpWithSlot()
    {
        ...
        if (SquadBData.Instance.IsSlotEmpty(selectingIndex)) {
            SquadBData.Instance.Occupie(sailor.Model.id, selectingIndex);
        } else
        {
           
            SquadBData.Instance.Swap(sailor.Model.id, SquadBData.Instance.OwnerOf(selectingIndex));
        }
